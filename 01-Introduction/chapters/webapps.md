...
<b> Shortcut:</b>  
$ <mark>docker rm -f \`docker ps -aq\`</mark>  
...
<b>Cattle method:</b>  
 ... Launch,   
$ <mark>for i in \`seq 1 5\`;do docker run -e AUTHOR="Container No=$i" -d -P dockersamples/static-site;done</mark>  
 ... play around ...  
 ... kill,     
$ <mark>docker rm -f \`docker ps -aq\`</mark>  
 ... bandwidth is cheap, cleanup aggressively to get in the automation habit,   
$ <mark>docker system prune -a</mark>   
 ... and dont forget old volumes,   
$ <mark>docker volume prune</mark>
...
<b>Now forget the Ubuntu images ... these are better</b>  
$ <mark>docker pull python:3.7-slim-stretch</mark>  
$ <mark>docker pull python:3.8-slim-buster</mark>  

<b>Excercise .. run them and spot at least 3 differences</b>  
...
- [2.0 Webapps with Docker](#20-webapps-with-docker)
  ...
  - [2.3.4 Push your image](#234-push-your-image)
  - [2.3.5 Dockerfile commands summary](#235-dockerfile-commands-summary)
...
<b> If you don't see cats</b>   
 Try this line in app.py ... <mark>images = ['https://cataas.com/cat?' + str(i) for i in range(100)]</mark>   


<b>Poor mans build cycle</b>  
$ <mark>name=mycats; docker rm -f \`docker ps --filter "name=$name" -aq\` 2>/dev/null; docker build -t local/"$name" . && docker run -p 8888:5000 -e FLASK_ENV=development -d --rm --name "$name" local/"$name"</mark>  
  ... change some code, repeat ...   
<b>Note: maybe not best practice, but it works fast ... and is fun :)</b>
...
## Next Steps
For the next step in the tutorial, head over to [3.0 Docker Compose](./dockercompose.md)
